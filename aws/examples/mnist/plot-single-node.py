...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=os.path.join(here, "results", "single-node"),
    )
    parser.add_argument(
        "--out",
        help="directory to save parsed results",
        default=os.path.join(here, "img", "single-node"),
    )
    return parser
...
def plot_results(df, outdir):
    ...
    make_plot(
        df,
        title="Total Times of Mnist on a Single Node",
        tag="mnist",
...
def parse_realtime(item):
    """
    Parse the realtime (time command wrapped)
    """
    lines = item.split("\n")
